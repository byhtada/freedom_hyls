...
//Program
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if (data.error == 0) {
                        ...
                        $('.marafon_name').text(data.material_name);
                    } else {
    ...
    var selected_day;
    var days = [];
    function btnDayClicked(btn){
        console.log("btn", btn);
        if (btn == "set"){
            console.log(selected_day);
            getDayInfoDetox(selected_day);
        }
    }
    function setDayPicker(marafon_day){
        if (days.length == 0) {
            var date1 = new Date();
            ...
            while (i > 0){
                ...
                var day_date1 = new Date(date1 - (marafon_day - i) * 24 * 60 * 60 * 1000);
                var month1 = day_date1.getMonth() < 10 ? "0" + (day_date1.getMonth() + 1) : day_date1.getMonth();
                var date_str = "(" + day_date1.getDate() + "." + month1 + "." + day_date1.getFullYear() + ")";

                days.push({
                    val: i,
                    label: i + " " + day_s + "  " + date_str
                });
                i -= 1;
            }
        }
    }
    $("#user_current_day").AnyPicker({
        mode: "select",
        ...
        buttonClicked: btnDayClicked,
        onChange: function(iRow, iComp, oSelectedValues)
        {
            selected_day = oSelectedValues.values[0].val;
    ...
    function setWakeUpInput(wake_up_hours_fact, wake_up_minutes_fact){
        ...
        selected_time_hour    = wake_up_hours_fact;
        selected_time_minute  = wake_up_minutes_fact;
    }
    ...
    $('#filed_no_snacking_fact, #filed_diet, #filed_tongue_day, #filed_tongue_night, #filed_phisic,#filed_read_material, #filed_therapy, #filed_asana, #filed_psy, #filed_half_bath_day, #filed_half_bath_night, #filed_ahimsa, #filed_satya, #filed_asteya, #filed_brahma, #filed_aparigraha, #filed_shaucha, #filed_santosha, #filed_tapah, #filed_svadhya, #filed_ishvara, #filed_social_practic, #filed_hyls_report').change(function() {
    ...
    function hide_all_detox_questions(){
    ...
    function userSaveDayDetox(day_num) {
        ...
        $.ajax({
            ...
            data: { query_update: "user_save_day_status",
                ...
                read_material:                $('#filed_read_material')                 .is(':checked'),
                ...
                hyls_report_fact:   $('#filed_hyls_report').is(':checked'),
    ...
    btns_question_djama.forEach(function(item){
        $(item.btn).click(function (){
            $(item.btn).prop('disabled', true);
            $.ajax({
                type: "GET",
                url:  api_url_full,
                data: { query_update: item.query,
                        djama_answer: $(item.field)         .is(':checked')
                },
                headers: {
                    'Authorization':'Token token=' + cookie_token,
                    'Content-Type':'application/x-www-form-urlencoded'
    ...
    $('#btn_question_wake_up_save').click(function (){
        ...
        if ($('#filed_question_wake_up_fact_hour').val()  != "" && $('#filed_question_wake_up_fact_minute').val()  != "" && $('#filed_question_wake_up_target_hour').val()  != "" && $('#filed_question_wake_up_target_minute').val( ) != "" && $('#filed_question_wake_up_step').val( ) != ""){
            $.ajax({
                type: "GET",
                url:  api_url_full,
                data: { query_update: "user_save_wake_up_answer",
                    ...
                    question_wake_up_step:      $('#filed_question_wake_up_step')       .val()
                },
                headers: {
                    'Authorization':'Token token=' + cookie_token,
                    'Content-Type':'application/x-www-form-urlencoded'
    ...
    $(document).on('click', '.link_detox_time',  function () {
        ...
        switch (parseInt(detox_type)){
            case 1:
                detox_last_date = 60 - detox_time - 2;
                break;
            case 2:
            ...
            case 3:
                ...
                break;
            case 4:
                detox_last_date = 60 - detox_time - 1 - 1 - 2 - 1 - 2 - 5;
                break;
        }
        ...
        for (i = 28; i < detox_last_date + 1; i++) {
            detox_start      += '<li><a href="#" class="link_detox_start" name="'+ i +'">' + i + " " + day_s + '</a></li>';
        }
    ...
    $(document).on('click', '.link_therapy_type',  function () {
        ...
        $('#btn_dd_therapy_type_settings').text($(this).text());
...
//Diary
    ...
    function get_practic_name_rus(practic){
        ...
        switch (practic) {
            ...
            case "social":          material_for_practic = "Контроль соц. сетей"; break;
            case "extra":           material_for_practic = "Избавление от лишнего"; break;
            case "parasites":       material_for_practic = "Избавление от слов-паразитов"; break;
            case "communication":   material_for_practic = "Общение без конфликтов"; break;
...
//Settings
    ...
    $('#btn_diet_edit').click(function (){
        $('#btn_diet_edit').prop('disabled', true);

        $.ajax({
            type: "POST",
            url:  api_url + "set_diet_active",
            data: { diet_active:  $('#filed_diet_settings').is(':checked')},
            headers: {
                'Authorization':'Token token=' + cookie_token,
                'Content-Type':'application/x-www-form-urlencoded'},
            success: function(data){
                $('#btn_diet_edit').prop('disabled', false);
                update_user_info();
    ...
    $('#btn_therapy_edit').click(function (){
        if (therapy_type != "") {
            $('#btn_therapy_edit').prop('disabled', true);

            $.ajax({
                type: "POST",
                url:  api_url + "detox_edit_therapy",
                data: { therapy_type:  therapy_type},
                headers: {
                    'Authorization':'Token token=' + cookie_token,
                    'Content-Type':'application/x-www-form-urlencoded'},
                success: function(data){
                    $('#btn_therapy_edit').prop('disabled', false);
                    update_user_info();
                    $('#page_user_settings').hide();
                    alert("Изменения приняты");},
                failure: function(errMsg) {alert(errMsg.toString());}
            });
        } else alert("Сначала выберите тип");
    });
    ...
    $('.btn_settings_practise').click(function (){
        ...
        console.log("practic_status " + practic_status);
    ...
    $(document).on('click', '.link_detox_type_edit',  function () {
        $('#btn_dd_detox_type_edit').text($(this).text());
        detox_type_edit = parseInt($(this).attr("name"));
    });
    $('#btn_detox_edit_type').click(function (){
        if (detox_type_edit !== 0){
            $.ajax({
                type: "GET",
                url:  api_url_full,
                data: { query_update:   "detox_edit_type",
                    detox_type_new: detox_type_edit},
                headers: {
                    'Authorization':'Token token=' + cookie_token,
                    'Content-Type':'application/x-www-form-urlencoded'
                },
            ...
            alert ("Сначала выберите вариант детокса")
        }
